...
it('Deals a card', () => {
  ...
  expect(
    fromHeartsReducer.getPlayerHand(state, testConstants.playerBob.id)
  ).toEqual([testConstants.cardAH]);
});
...
it('Plays a card', () => {
  ...
  expect(fromHeartsReducer.getCurrentTrick(state)).toEqual([
    { card: testConstants.cardAH, playerID: testConstants.playerBob.id }
  ]);
});
